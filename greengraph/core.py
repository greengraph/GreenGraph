...
class GreenMultiDiGraph(nx.MultiDiGraph):
    ...
    Every **node** must have a `type` attribute, which can be one of the following:
    ...
    |-------------|--------|----------------------------------------------------------------|
    | uuid        | str    | Unique random identifier                                       |
    ...
    Every **edge** must have a `type` attribute, which can be one of the following:

    **Flow Edges** (`type='flow'`)

    !!! Tip
        This can describe a flow of material, energy, money, etc.

    | Required Attribute | Type   | Description                                                    |
    |--------------------|--------|----------------------------------------------------------------|

    **Characterization Edges** (`type='characterization'`)

    | Required Attribute | Type   | Description                                                    |
    |--------------------|--------|----------------------------------------------------------------|

    **Concordance Edges** (`type='concordance'`)

    | Required Attribute | Type   | Description                                                    |
    |--------------------|--------|----------------------------------------------------------------|
    
    Of course, it is possible to add other attributes to the edges, but these are not required.
    Some other useful attributes, in the context of life-cycle assessment and
    supply chain analysis, are:
    ...
    Warnings
    --------
    This class is overriding the NetworkX methods for adding nodes and edges (`networkx.MultiDiGraph.add_edge`, `networkx.MultiDiGraph.add_node`, etc.).
    This ensures that the nodes and edges have the required attributes. Be aware that this may cause issues with some NetworkX functions that do not expect this behavior.

    For example, the [`networkx.from_numpy_array`](https://networkx.org/documentation/stable/reference/generated/networkx.convert_matrix.from_numpy_array.html) 
    might throw an error. Consider using the [`greengraph.utility.graph.graph_from_matrix`][] utility function instead.
    ...
    def _validate_node_attributes(
        ...
        dict_attr : dict
            A dictionary of attributes to validate.  
            This dictionary must contain the `type` attribute and the required attributes for the node type.

        Raises
        ------
        ValueError
            If the node is not in the graph and no attributes are provided.  
            If the node does not have a `type` attribute.
            If the node has an invalid `type` attribute.
        """
    ...
    def _validate_edge_attributes(
        ...
        dict_attr : dict
            A dictionary of attributes to validate.  
            This dictionary must contain the `type` attribute and the required attributes for the edge type.
        
        Raises
        ------
        ValueError
            If the edge is not in the graph and no attributes are provided.  
            If the edge does not have a `type` attribute.
            If the edge has an invalid `type` attribute.
        """
        ...
        if set_missing_attributes:
            raise ValueError(f"Edge is missing required attributes: {set_missing_attributes}.")
